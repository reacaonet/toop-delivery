...
import { lastBookingHistoricPassenger } from '../../../services/provider/booking/historic';
...
export const SelectDestiny = () => {
  ...
  const isInitialLocationUpdate = useRef(false);
  ...
  const [historic, setHistoric] = useState<any>([defaultPlaces]);
  const [current, setCurrent] = useState<any>();

  useEffect(() => {
    (async () => {
      if (user?._id && isInitialLocationUpdate.current === false && actualLocation?.latitude === 0) {
        getCurrentPosition(setActualLocation);
        isInitialLocationUpdate.current = true;
      }
    })();
  }, [user?._id, actualLocation]);



  useEffect(() => {
    const { latitude = 0, longitude = 0 } = actualLocation;
    ...
    async function getLocalName() {
      ...
      let geoLocation: any = await StorageGet('@user_loc_current');
    ...
    if (user?._id && actualLocation && actualLocation.latitude !== 0 && actualLocation.longitude !== 0) {
      getLocalName();
      setLinkToFranchise(latitude, longitude);
    }
  }, [user?._id, actualLocation?.latitude, actualLocation?.longitude]);

  useEffect(() => {
    if (user?.passenger?._id) {
      lastBookingHistoricPassenger({
        passenger: user?.passenger?._id,
      }).then(result => {
        if (result && Array.isArray(result) && result.length > 0) {
          const last: any = defaultPlaces;

          for (const item of result) {
            last.push({
              _id: item._id,
              description: ' ',
              value: item.destiny[0].address,
              key: 'historic',
              icon: 'history',
              address: item.destiny[0].address,
              latitude: item.destiny[0].coordinates[1],
              longitude: item.destiny[0].coordinates[0],
            });
          }
          setHistoric(last);
        } else {
          setHistoric(defaultPlaces);
        }
      });
    }
  }, [user?.passenger?._id]);
  ...
  }, [route.params?.destiny, route.params?.destinyType]);
  ...
  const setLinkToFranchise = async (latitude: number, longitude: number) => {
    if (user?.passenger) {
      ...
      if (respLink && respLink._id) {
        ...
        listSettings(respLink._id).then(result => {
          if (
            result &&
