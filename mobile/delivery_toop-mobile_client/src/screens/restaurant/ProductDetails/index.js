...
import CartItemComplement from '../ProductDetails/cartItemComplement';
...
const ProductDetails = ({navigation, route, onAddToCart, onGetToCart}) => {
    ...
    useCallback(() => {
      if (route.params && route.params.cartItem) {
        ...
        if (paramsItem) {
          ...
          if (paramsItem && paramsItem?.comment) {
            setComment(paramsItem?.comment);
          }
        }
      }
    }, [idProduct, route.params]),
  );
  ...
  /*
   * complement control functions*/
  const changeComplementItem = (
    id,
    qtd,
    complement_id,
    complement_amountMax,
  ) => {
    const indexComplement = complement.findIndex(
      prod => prod.complement._id === complement_id,
    );
    const newComplement = [...complement];
    const index = newComplement[indexComplement].products.findIndex(
      cp => cp._id === id,
    );
    newComplement[indexComplement].products[index] = {
      ...newComplement[indexComplement].products[index],
      quantity: qtd,
    };
    ...
    setCheckMultile(
      id,
      complement_id,
      newComplement[indexComplement].products[index].price,
      complement_amountMax,
      qtd,
    );
  };
  const removeQty = (id, qtd, complement_id) => {
    const indexComplement = complement.findIndex(
      prod => prod.complement._id === complement_id,
    );
    const newComplement = [...complement];
  ...
  const setCheckMultile = (id, groupId, priceCheck, qtyComplement, qtyItem) => {
    ...
    if (checkIndex < 0) {
      tmpCheck = [
        ...check,
        {group: groupId, id, price: priceCheck * qtyItem, quantity: qtyItem},
      ];
      setCheck(tmpCheck);
    } else {
      if (qtyItem === 0) {
          ...
          <View style={styles.descriptContent}>
            <View style={styles.cardQtd}>
              <View style={styles.boxPrice}>
                <Text numberOfLines={4} style={styles.titleProduct}>
                  {product.name}
                </Text>
                  ...
                  {product.pricePromotion ? (
                    <>
                      ...
                      <Text style={styles.txtValue}>
                        {formatMoney(product.price, configurations?.coin)}
                      </Text>
                    </>
                  ) : product.price !== 0 ? (
                    <Text style={styles.pricePromotion}>
            ...
            return (
              <View key={comp.complement._id} style={styles.boxOptions}>
                ...
                <View style={styles.boxComplements}>
                  {comp.complement.isQuantified
                    ? comp.products?.map(item => (
                        <View
                          style={
                            showCheck(item._id)
                              ? styles.complementItemSelected
                              : styles.complementItem
                          }>
                          ...
                          <View>
                            <CartItemComplement
                              qtd={item.quantity ? item.quantity : 0}
                              add={changeComplementItem}
                              remove={changeComplementItem}
                              total={7}
                              id={item._id}
                              complement_id={comp.complement._id}
                              complement_amountMax={comp.complement.amountMax}
                              disposed={true}
                            />
                          </View>
                        </View>
