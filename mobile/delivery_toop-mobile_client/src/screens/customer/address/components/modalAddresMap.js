...
const ModalAddresMap = ({
  ...
  const [addressRoute, setAddressRoute] = useState(
    edit?.addressRoute || address?.addressRoute || '',
  );
  const [isResults, setIsResults] = useState(true);
  const [number, setNumber] = useState(
    edit?.streetNumber?.toString() || address?.streetNumber || '',
  );
  const [district, setDistrict] = useState(edit?.district?.toString() || '');
  ...
  const save = async () => {
    try {
      ...
      if (!district || district.trim().length < 3) {
        Alert.alert(
          'Verifique os Dados enviados!!',
          'Informe um Setor/Bairro válido',
        );
        setLoad(false);
        return;
      }

      if (!addressRoute || addressRoute.trim().length < 3) {
        Alert.alert(
          'Verifique os Dados enviados!!',
          'Informe o endereço da rua',
        );
        setLoad(false);
        return;
      }
      ...
      let result = await createDeliveryAddress({
        ...
        addressRoute: `${addressRoute}`.replace(',', ''),
                ...
                <View style={{flex: 1}}>
                  <View style={styles.txtAddressHeader}>
                    ...
                    <TextInput
                      ...
                      value={addressRoute}
                      onChangeText={setAddressRoute}
                      keyboardType={'default'}
                    />
                    <Text numberOfLines={1} style={styles.txtError}>
                      {district && district.length <= 3
                        ? 'Informe o Bairro com pelo menos 3 caracteres'
                        : null}
                    </Text>
