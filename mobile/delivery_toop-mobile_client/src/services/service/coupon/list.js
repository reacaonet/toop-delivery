...
const listCompanyCoupons = async (id, params) => {
  try {
    const getQuery = queryString(params);

    const response = await api.get(`/coupon/companyCoupons/${id}?${getQuery}`);
    const data = response.data;
    if (data && data.companys) {
      return data.companys;
    }
...
const listCompanyCouponsAvailable = async (
  id,
  subTotal,
  person,
  params = {},
  ...
  try {
    ...
    const response = await api.get(
      `/coupon/companyCouponsAvailable/${id}?subTotal=${subTotal}&person=${person}&${getQuery}`,
    );
    const data = response.data;
    if (data && data.companys) {
      return data.companys;
