...
const google = async user => {
  try {
    ...
    const userSearch = await listCustomerSearch({
      email: user.email,
      loginType: 'GOOGLE_EMAIL',
      idToken: user?.idToken,
    });

    if (userSearch && userSearch.token && userSearch.refresh) {
      await defineSessionToken({
        token: userSearch.token,
        refresh: userSearch.refresh,
      });
    }
...
const apple = async appleInfo => {
  try {
    ...
    if (userSearch !== null && userSearch.person) {
      ...
      await StorageSet('providerUser', 'apple');
    ...
    let person = await createPerson({
      name:
        userAppleAuth.displayName === null
          ? userAppleAuth.email.split('@')[0]
          : userAppleAuth.displayName,
      email: userAppleAuth.email.toLowerCase(),
      devices: [uniqueId],
      status: true,
    });
...
const facebook = async user => {
  try {
    ...
    if (userSearch && userSearch.person) {
      if (userSearch.device !== uniqueId) {
        await updateCustomer(userSearch._id, {
          device: uniqueId,
        });
  ...
  try {
    ...
    if (auth) {
      ...
      if (currentUser) {
        await auth()
          .signOut()
          .then(() => console.log('Clean Auth Firebase'));
      }
    }
    return true;
  } catch (err) {
...
const defineSessionToken = async storage => {
