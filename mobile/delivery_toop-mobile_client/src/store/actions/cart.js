    ...
    StorageClean('cart').then(() => {
      isAuthenticated()
        .then(user => {
          ...
          if (customer && customer._id && !guest) {
            listCart(customer._id, company, {status: 'pending'})
              .then(result => {
                if (result && result[0] && result[0]._id) {
                  StorageSet('cart-atual', result[0]);
                  cartCurrent(result[0]._id, params).then(itens => {
                    StorageSet('cart', {...itens, company: result[0].company});
                    dispatch(
                      getToCartDispatch(company, {
                        ...itens,
                  ...
                  dispatch(
                    getToCartDispatch(company, {cart: [], status: null}),
                  );
                  StorageSet('cart', []);
                }
              })
    ...
    StorageGet('cart').then(itensCart => {
      ...
      if (cart && cart.length > 0) {
        ...
        createCartItem(cartId, product._id, {
          ...
          radio,
          type:
            company.shoppingFlow === 'PRODUCT' ? 'supermarket' : 'restaurant',
          comment,
          ...pizzaPayload,
        })
          .then(_result => {
            // console.log('criando carrinho ...', _result);
          ...
          .catch(err => {
            console.log('Falha ao criar carrinho', err);
            dispatch(loadToCart(false));
          });
      } else {
        ...
        isAuthenticated().then(user => {
          ...
          if (customer && customer._id) {
            createCart(customer._id, company._id, {})
              .then(resultCart => {
                ...
                if (resultCart && resultCart._id) {
                  createCartItem(resultCart._id, product._id, {
                    ...
                    .then(_itemResult => {
                      cartCurrent(resultCart._id, params)
                        .then(itens => {
                          StorageSet('cart', itens).then(() =>
                            dispatch(listToCart(itens)),
                          );
                  ...
                  listCart(customer._id, company._id, {status: 'pending'})
                    .then(result => {
                      if (result && result[0]) {
    ...
    StorageGet('cart').then(itensCart => {
      ...
      if (cart && cart.length > 0) {
        ...
        if (index >= 0) {
          ...
          if (qtd <= 0) {
            deleteCartItem(cart[index].cartId, cart[index].cartItemId)
              .then(() => {
