...
const Add: React.FC = () => {
  ...
  const loadComplements = () => {
    listComplements(company._id, params?.product?._id).then((response) => {
      setComplements({change: true, data: response});
      setName(params?.product?.name);
      setDescription(params?.product?.description);
      setCategory(
        categories.find((i) => i.value === params?.product?.category),
      );
      setPicture({uri: params?.product?.images[0] ?? ''});
      setCodPdv(params?.product?.codPdv);
      setPrice(maskRealBeautify(params?.product?.price ?? 0, true));
      setPricePromotion(
        maskRealBeautify(params?.product?.pricePromotion ?? 0, true),
      );
    });
  };
    ...
    try {
      ...
      updateProduct(company._id, params?.product?._id, data)
        ...
        .catch((err) => {
          ...
          setSaving(false);
          Alert.alert('Oops', 'Não possível atualizar o produto');
        });
    } catch (err) {
  ...
  const loadCategories = () => {
    listCategory(company._id, '').then((response: any) => {
      ...
      setCategories(cats);
      setCategory(cats.find((i: any) => i.value === params?.product?.category));
      loadComplements();
    });
  };
  ...
  React.useEffect(() => {
    if (!params?.product?._id) return navigation.goBack();
    loadComplements();
  }, [categories.length, params?.product?._id]);
  ...
  return (
    <Container>
      ...
      <Touch style={{marginTop: 10}} onPress={handleSave}>
        <TitleTouch>Atualizar produto</TitleTouch>
      </Touch>
    </Container>
  );
};
