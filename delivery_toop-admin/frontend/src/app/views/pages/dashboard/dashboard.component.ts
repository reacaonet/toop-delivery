...
export class DashboardComponent implements OnInit {
	...
	async ngOnInit(): Promise<void> {
		...
		try {
			...
			let order: number = 0;
			let orderSuccess: number = 0;
			let tickets = 0;

			let response: any = await this.shoppingService
				.getGraphicCartSales()
				.toPromise();
			...
			let uniqueUser: any = await this.AccessFlowService.getUniqueAccessFlow(
				30
			).toPromise();

			this.totalUniqueUser = uniqueUser?.data?.count;

			for await (const item of response) {
				tickets += Number(item.totalOrder);
				order += Number(item.totalSales);
				orderSuccess += Number(item.totalApproved);
			}

			this.averageTickets = Number(order.toFixed(2)) / tickets;
			this.totalOrder = order;
			this.totalOrderSuccess = orderSuccess;
			this.changeDetectorRefs.detectChanges();
		} catch (err) {
