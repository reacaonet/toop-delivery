...
let tip = null;
let typePayment = "MONEY";
const valueMin = 0.09;

/**
 * url - /v2/payment/money/send/cart/:cartId
 * POST
 * - customer
 */
const sendMoney = async (req, res) => {
  try {
    ...
    const { customer, ipAddress, coupon, valueTip, cashChange, typeSchedule, deliveryFree, usedCashback } = req.body;
    tip = null;
    ...
    // Troco
    let valueCashChange = 0;
    if (cashChange) {
      valueCashChange = `${cashChange}`.replace(/[^0-9.,]+/g, "").replace(",", ".");
    }
    ...
    path: '/src/controllers/v2/Payment/PaymentMethod/Money/MoneyController.js',
    error: err?.message,
    method: 'sendMoney',
    type: 'error',
    level: 0,
