...
module.exports = async (req, res) => {
  try {
    ...
    list = await CampaignModel.find(filter)
      .limit(parseInt(limit))
      .skip(parseInt(page) * parseInt(limit));
    let numTotal = await CampaignModel.find().countDocuments();
    return res.json({ list, total: numTotal })
  } catch (dadosDoErro) {
  await LogModel.create({
    path: 'src/controllers/marketing/Campaign/PaginatorController.js',
