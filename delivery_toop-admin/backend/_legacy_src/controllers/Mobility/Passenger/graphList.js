...
module.exports = async (req, res) => {
  try {
    ...
    const list = await PassengerModel.aggregate([
      {
        $group: {
          _id: {
            month: {
    ...
    const total = await PassengerModel.aggregate([
      {
        $group: {
          _id: "id",
          enable: {
  ...
  await LogModel.create({
    path: 'src/controllers/Mobility/Passenger/graphList.js',
  ...
  console.log(`Log de erro criado com sucesso.`);
    ...
    return res.status(400).send({
      message: "Falha ao lista todos os items",
      Error: dadosDoErro.message,
    });
  }
};
