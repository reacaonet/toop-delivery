...
module.exports = async (req, res) => {
  try {
    ...
    const novoRegistro = await Department.findOneAndUpdate({
      _id: id
    }, data, {
      upsert: true,
      new: true
    ...
    if (data && data.suggesteds && typeof data.suggesteds === 'object') {
      updateIntegration(data.suggesteds, req);
    }
  ...
  await LogModel.create({
    path: 'src/controllers/Shopping/Department/UpdateController.js',
...
const updateIntegration = async (suggested, req) => {
  try {
    let querySuggested =
