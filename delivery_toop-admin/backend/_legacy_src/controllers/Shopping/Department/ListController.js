...
const list = async (req, res) => {
  try {
    const { suggested, barcode, all, franchise, company, term } = req.query;
    ...
    if (all) {
      ...
      let response = await Department.find(filter);
    ...
    let formatTxt = decodeURIComponent(suggested).toLowerCase();
    filter.suggesteds = { $regex: ".*" + formatTxt + ".*", $options: "i" };

    const listDepartments = await Department.find(filter);
    return res.status(200).send(listDepartments);
  } catch (err) {
    await LogModel.create({
      path: 'src/controllers/Shopping/Department/ListController.js',
      error: err?.message,
      method: 'list',
      type: 'error',
      level: 0,
