...
const list = async (req, res) => {
  try {
    ...
    if (companyType === "supermarket") {
      response = await supermarket(filter, searchText, limitCompany, limitProduct);
      responseS = await searchCompanySupermarket(searchText, latitude, longitude);
      response = responseS.concat(response);
    } else if (companyType === "restaurant" || companyType === "accessories") {
      response = await restaurant(searchText, filterRestaurant, companyType);
      responseS = await searchCompanyRestaurant(searchText, latitude, longitude, companyType);
      response = responseS.concat(response);
    }
    ...
    await LogModel.create({
      path: 'src/controllers/Search/v1/ListController.js',
      error: err?.message,
      method: 'list',
      type: 'error',
      level: 0,
...
const restaurant = async (searchText, filterRestaurant, companyType) => {
  const path = companyType === "restaurant" ? "foodCategory" : "accessoriesProduct";
  try {
...
const searchCompanyRestaurant = async (searchText, latitude, longitude, companyType) => {
  try {
    let filter = {};
    filter.type = companyType;
    filter.status = true;
