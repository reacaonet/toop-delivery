...
module.exports = async (req, res) => {
  try {
    ...
    const list = await CashbackCampaignModel.find(filter)
      .populate("companies")
      .skip(parseInt(pageIn) * parseInt(pageOut))
      .limit(parseInt(pageOut));

    let numTotal = await CashbackCampaignModel.find(filter).countDocuments();

    return res.json({ list, total: numTotal });
  } catch (dadosDoErro) {
  await LogModel.create({
    path: 'src/controllers/Cashback/Campaign/PaginatorController.js',
