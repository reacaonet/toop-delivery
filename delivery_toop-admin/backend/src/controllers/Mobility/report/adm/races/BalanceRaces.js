...
const balanceReportRaces = async (request, reply) => {
  try {
    const { startDate, endDate, status, passenger, typePayment } = request.query;
    ...
    await LogModel.create({
      path: 'src/controllers/Mobility/report/adm/races/BalanceRaces.js',
      error: err?.message,
      method: 'balanceReportRaces',
      type: 'error',
      level: 0,
      origin: 'backend',
      request: {
...
module.exports = balanceReportRaces;
