...
module.exports = async (req, res) => {
  try {
    ...
    data.isPaused = (typeof data.isPaused === "string" && data.isPaused === "") || data.isPaused === null ? false : data.isPaused;
    // Trata status
  ...
  await LogModel.create({
    path: 'src/controllers/Food/Product/CreateController.js',
...
const createProduct = async product => {
  return new Promise(async (resolve, reject) => {
    try {
      const data = {
        ...
        price: product.price,
        pricesSizesPizzas: product.pricesSizesPizzas,
        company: product.company,
        pricePromotion: typeof product.pricePromotion === "number" ? product.pricePromotion : undefined,
        codPdv: product.codPdv,
      };
      const productInsert = await ProductModel.create(data).catch(err => console.log("Create productModel Error", err));
  ...
  await LogModel.create({
    ...
    request: {
      ...
      franchise: resolve?.franchise,
      company: resolve?.company,
      params: resolve?.params,
      body: resolve?.body,
      query: resolve?.query,
      heders: resolve?.heders,
...
const createComplementsAndItems = async (productId, companyId, data) => {
  console.log(productId, companyId, data);
    ...
    try {
      for await (const comp of data) {
        const complementPayload = {
          ...
          isQuantified: comp.isQuantified !== "" && JSON.parse(`${comp.isQuantified ? comp.isQuantified : true}`) === true ? true : false,
          isPaused: comp.isPaused !== "" && JSON.parse(`${comp.isPaused}`) === true ? true : false,
          company: companyId,
        };

        const insertComplement = await ProductComplementModel.create(complementPayload).catch(err => console.log("Create Complement Error", err));

        if (comp.items && Array.isArray(comp.items)) {
          for await (const item of comp.items) {
            const itemPayload = {
              foodProductComplement: insertComplement._id,
              name: item.name,
              codPdv: item.codPdv,
            ...
            await ProductComplementItemModel.create(itemPayload).catch(err => console.log("Create Item Error", err));
          }
        }
      }
      return resolve({});
