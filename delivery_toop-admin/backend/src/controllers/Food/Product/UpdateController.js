...
module.exports = async (req, res) => {
  try {
    ...
    await removeComplementsAndItem(product._id, data, req);

    // Save/Update complements and items
    await upsertComplementsAndItems(product._id, data, req);
  ...
  await LogModel.create({
    path: 'src/controllers/Food/Product/UpdateController.js',
...
const upsertComplementsAndItems = async (productId, data, req) => {
  ...
  await LogModel.create({
    path: 'src/controllers/Food/Product/UpdateController.js',
    error: dadosDoErro?.message,
    method: 'upsertComplementsAndItems',
    type: 'error',
    level: 0,
    origin: 'backend',
    request: {
...
const removeComplementsAndItem = async (productId, data, req) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (complementsAtual && Array.isArray(complementsAtual) && complementsAtual.length > 0) {
        for await (const comp of complementsAtual) {
          ...
          if (check && Array.isArray(check) && check.length <= 0) {
            ...
            await ProductComplementItemModel.deleteMany({foodProductComplement: comp._id})
          } else {
            // Check Items
            // Get all items atual from complements
            const complementsItemAtual = await ProductComplementItemModel.find({
              foodProductComplement: comp._id
            }, {
              _id: 1
            }).lean();
  ...
  await LogModel.create({
    path: 'src/controllers/Food/Product/UpdateController.js',
    error: dadosDoErro?.message,
    method: 'removeComplementsAndItem',
    type: 'error',
    level: 0,
    origin: 'backend',
    request: {
