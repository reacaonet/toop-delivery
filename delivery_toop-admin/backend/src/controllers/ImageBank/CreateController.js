...
module.exports = async (req, res) => {
  try {
    ...
    const imageBank = await ImageBank.create(data);

    return res.send({
      status: 200,
      message: "Banco de imagens criada com sucesso",
      data: imageBank
    });
  } catch (dadosDoErro) {
  await LogModel.create({
    path: 'src/controllers/ImageBank/CreateController.js',
  ...
  console.log(`Log de erro criado com sucesso.`);
    ...
    return res.status(400).send({
      message: "Falha ao criar Banco de imagens",
      error: dadosDoErro
    });
  }
};
