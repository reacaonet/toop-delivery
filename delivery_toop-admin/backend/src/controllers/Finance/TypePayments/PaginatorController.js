...
module.exports = async (req, res) => {
  try {
    ...
    list = await FncTypePayments.find(filter)
      .limit(parseInt(pageOut))
      .skip(parseInt(pageIn) * parseInt(pageOut));
    let numTotal = await FncTypePayments.find().countDocuments();
    return res.json({ list, total: numTotal })
  } catch (dadosDoErro) {
  await LogModel.create({
    path: 'src/controllers/Finance/TypePayments/PaginatorController.js',
